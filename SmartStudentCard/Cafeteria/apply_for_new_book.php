<?php
	...
	if (isset($_POST['issued_book'])) {
		...
		if($s_id!=''){
			...
			if ($DB->valid_book($accession_id)){
				if($DB->library_service_check($s_id)){
					if($DB->library_service_provide($s_id,$accession_id,$issue_date,$user)){
						$msg="Book Issued Succesfully";
					}else $msg="Somthing went wrong, Please try again!";
				}else $msg = "Already 3 book issued for this Student.";
			}else $msg = "Book is not available or invalid accession number !";
	...
	<br>
		...
		<div class="LibRightPanel">
			...
			<div class="container">
			  ...
			  <br>
			  <div class="row">
			    <input type="submit" name="issued_book">
			  </div>
			  </form>
			</div>
